...
需要补充的代码如上，通过 `page2kva` 获取源页面和目标页面的内核虚拟地址，然后使用 `memcpy` 将源页面内容复制到目标页面，将父进程的内存空间复制给子进程，最后通过`page_insert`将新页面的物理地址映射到目标进程的虚拟地址 `start`。
...
1. **页表标记共享页面**：父进程和子进程共享页面时，将页面的权限设置为只读。
2. **写入时触发缺页异常**：如果父进程或子进程尝试写入共享页面，处理器会触发页访问异常（缺页中断）。
3. **异常处理复制页面**：
   - 检测到是写入访问引发的异常。
   - 为当前进程分配一个新的物理页面。
   - 将原页面的内容复制到新页面。
   - 将新的物理页面映射回当前进程，同时权限改为可写。
...
1. **`copy_range` 函数**：在进程创建时，设置页面共享或复制页面内容。
2. **`do_pgfault` 函数**：在页面写入时处理缺页异常，实现页面的实际复制。

`copy_range` 函数负责将一个地址空间的内存范围（`[start, end)`）复制到另一个地址空间中。通过 `share` 参数决定是直接共享页面（COW），还是完全复制页面内容。
...
`do_pgfault` 函数在页面写入触发的缺页异常中，处理 COW 逻辑。如果页面只有一个引用，直接修改页表权限，将页面变为可写；如果页面有多个引用：分配新页面，复制原页面内容到新页面，然后更新页表项，映射到新页面，并设置可写权限。

最终结果：

![818a08badca00274d8ec432f66774ca](lab5 report.assets/818a08badca00274d8ec432f66774ca.png)

> 说明该用户程序是何时被预先加载到内存中的？与我们常用操作系统的加载有何区别，原因是什么？

本次实验是把用户准备的二进制程序编译到了内核镜像中，也就是在内存中创建了一个大空间，将这个文件以二进制形式保存在了这一片内存区域中。

而常用操作系统是通过以下步骤来加载用户程序的：

1. **解析可执行文件**：操作系统读取可执行文件（如 ELF 格式），提取文件头和程序头信息，了解程序的各个段（代码段、数据段等）。
2. **分配虚拟内存**：为进程分配虚拟地址空间，准备好代码段、数据段、堆、栈等。
3. **加载程序段到内存**：将可执行文件的各个段（如代码段、数据段）加载到进程的虚拟内存中，BSS 段清零。
4. **设置虚拟内存映射**：通过页表将虚拟地址映射到物理地址，并设置内存权限（如读写执行权限）。
5. **初始化执行环境**：为程序设置栈、堆、文件描述符、环境变量等，准备好执行所需的资源。
6. **跳转到程序入口点**：设置程序的入口点（通常是 `main` 或 `_start`），并开始执行程序。
